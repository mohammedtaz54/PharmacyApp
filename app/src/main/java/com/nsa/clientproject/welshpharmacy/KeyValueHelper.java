package com.nsa.clientproject.welshpharmacy;

/**
 * Created by c1749909 on 16/03/2018.
 */

public class KeyValueHelper {
/*  default settings state for widgets
    true = ON
    false = OFF */

    public static final String KEY_BLOODPRESSURE_CHECKBOX = "checkbox_bloodpressure";
    public static final boolean DEFAULT_BLOODPRESSURE_CHECKBOX = false;

    public static final String KEY_FLUSHOT_CHECKBOX = "checkbox_flushot";
    public static final boolean DEFAULT_FLUSHOT_CHECKBOX = false;

    public static final String KEY_POSTCODE_TEXT = "edittext_postcode";
    public static final String DEFAULT_POSTCODE_TEXT = "";

    public static final String KEY_MAXDISTANCE_TEXT = "edittext_maxdistance";
    public static final String DEFAULT_MAXDISTANCE_TEXT = "";

}
